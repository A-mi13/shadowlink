...
| *(нет флага)* `rotationWatchdogTick` | **500ms** + джиттер `uniform[0,tick)` | период свипа = **квант фазы новых TLS на wire**. Замер 08-13: 588 age-ротаций в одной фазе 5s-сетки (σ 0.0001s), 92.2% connect в одном 1s-окне. Три итерации: (1) ручка `SWEEP_PHASE_JITTER` через `nextDrainAttemptNs` — **откачена**, её читает только свип, дренаж уезжал на следующий тик в ту же фазу; (2) срезка 5s→500ms — линия 0.2Гц убита (R 1.0000→0.0096), но пик **переехал на 2Гц** (R=0.8657); (3) `time.Timer` с перевзводом `tick+uniform[0,tick)` — решётки не остаётся. Джиттер сэмплируется ЗАНОВО каждый взвод. Бюджет: `sweep`=`sweepWorstCase()`=4×tick. ⚠ Критерий — **R<0.2 по `reader started`**, а не доля в модальном окне (та падает при любом тике <1s, т.е. измеряет ручку) |
