...
| *(нет флага)* `rotationWatchdogTick` | **500ms** + джиттер `uniform[0,tick)` | период свипа = **квант фазы новых TLS на wire**. Замер 08-13: 588 age-ротаций в одной фазе 5s-сетки (σ 0.0001s), 92.2% connect в одном 1s-окне. Три итерации: (1) ручка `SWEEP_PHASE_JITTER` через `nextDrainAttemptNs` — **откачена**, её читает только свип, дренаж уезжал на следующий тик в ту же фазу; (2) срезка 5s→500ms — линия 0.2Гц убита (R 1.0000→0.0096), но пик **переехал на 2Гц** (R=0.8657); (3) `time.Timer` с перевзводом `nextWatchdogWakeup()`=`tick+uniform[0,tick)` — решётки не остаётся **у этого цикла**. Джиттер сэмплируется ЗАНОВО каждый взвод. Бюджет: `sweep`=`sweepWorstCase()`=4×tick. ⚠ Критерий — **R<0.2 по `reader started`**, а не доля в модальном окне (та падает при любом тике <1s, т.е. измеряет ручку). ⚠ **Развязан только момент НОВОГО СОЕДИНЕНИЯ** (ревью 08-14): `startDrain` зовётся из свипа и сразу спавнит `connectReserveSlot`, а фактический close делает `drainWatchdog` по своему жёсткому `time.NewTicker(drainPollInterval=500ms)` (`ws_pool_drain.go:805`) — вторая сетка 2Гц на wire жива, и замер R=0.8657 брался «по моменту дренажа», т.е. по ней. Для close нужна отдельная правка |
