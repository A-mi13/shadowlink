...
| *(нет флага)* `rotationWatchdogTick` | **500ms** + джиттер `uniform[0,tick)` | период свипа = **ЯКОРЬ ФАЗЫ ВСЕГО ПУЛА**. ✅ Замерено 2026-08-14 (прогон 161915, 4ч): фаза развязана ЦЕЛИКОМ, R на уровне шума (0.025) по всем событиям — `reader started` 0.0330, `drain started` 0.0464, close 0.0447 (против 0.59 / 0.95 / 0.97 в базе). Слепой скан 0.05-6.0s: max R=0.063. ⚠ Момент close развязался НЕ своим джиттером: сетка `drainPollInterval` жива на 100% относительно старта дренажа (R=1.0000, 1525/1525 в одном 50ms-бине), но её фаза наследуется от `startDrain`, а тот из джиттерованного свипа. Возврат к `time.NewTicker` здесь немедленно проявит решётку 500ms в close БЕЗ правок в дренаже. Бюджет: `sweep`=`sweepWorstCase()`=4×tick. Критерий — **R<0.2**, а не доля в модальном окне (та падает при любом тике <1s) |
...
| *(наблюдаемость)* `sticky_exposure_over_82s_ms` | — | накопленный перебег соединений за 82s, в мс. Заведён 2026-08-14: после закрытия утечки ячеек sticky остался **единственным** значимым источником хвоста — 77% экспозиции >82s (196.8 из 254s за 4ч), 12 из 17 соединений >85s, max 102.6s. ⚠ **`drain_duration` в ветке `age_backstop` — константа** (все 50 teardown дали ровно 25s = `stickyMaxDrainAge`) и о риске молчит: читать **`slot_age`**. Механизм: `byte_budget`-дренаж (38 из 50) стартует в любом возрасте (p50 34.2 / max 77.6s), sticky добавляет +25s → потолок = старт+25s. Вред не наступил: ~1 ожидаемый рез за 4ч, наблюдалось 0 |
