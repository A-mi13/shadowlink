...
type RejectionStats struct {
	...
	TooYoung int
	// AboveTeardown — возраст СТАРШЕ полного бюджета жизни ячейки
	// (worstCaseTeardown). Такое наблюдение не про посредника: оно означает, что
	// НАШ механизм ротации не сработал, — а слот, который мы обязаны были снять
	// сами, не мог быть срезан на нашем пороге.
	//
	// Заведено по прогону 2026-08-17. Прежний filterNoise отсеивал шум только
	// СЛЕВА, поэтому наблюдение с CloseKind="close_other" и возрастом 293 с
	// проходило как свидетельство о цензоре. Источником были не резы, а заморозка
	// вывода в консоль (QuickEdit в PowerShell, 428 с, 17:47:09–17:54:17): пять
	// наблюдений 177–505 с влетели в ринг за две секунды в 17:54:17.
	//
	// Цена: samples_used 8→14, CV возраста 0.081 → 0.741 (впятеро), ложный WARN
	// `rotation budget reached by observed slot lifetimes` с
	// observed_age_p90=2m57.11s. Порог 68 с не пострадал СЛУЧАЙНО — p10 удержали
	// нормальные наблюдения. Опасность не в пороге: CV — та величина, по которой
	// Infer ВЫБИРАЕТ ОСЬ (axisSeparationFactor), то есть одна заморозка вывода
	// способна перевернуть выбор age → down_bytes или обнулить его в
	// AxisUnknown, после чего адаптация перестаёт исполняться.
	AboveTeardown int
...
func (r RejectionStats) Total() int {
	return r.ZeroAge + r.LocalClose + r.Timeout + r.TooYoung + r.Planned + r.AboveTeardown
}
...
//
// Верхняя граница правдоподобия ВЫКЛЮЧЕНА (эквивалент teardownMs = 0). Сигнатура
// сохранена ради вызывающих в тестах; продовый путь ходит через
// InferWithMinAgeAndTeardown.
func (r *Recorder) InferWithMinAge(minAgeMs int64) Verdict {
	return r.InferWithMinAgeAndTeardown(minAgeMs, 0)
}

// InferWithMinAgeAndTeardown — Infer с обеими границами правдоподобия
// наблюдения: снизу minAgeMs (`ageCutMinAge`), сверху teardownMs
// (`worstCaseTeardown()`), оба в мс.
//
// # Почему верхняя граница передаётся снаружи, а не считается здесь
//
// Ровно по той же причине, что и minAgeMs: `worstCaseTeardown()` живёт на
// WSPoolTransport и складывается из пяти конфигурируемых слагаемых
// (base + stagger + sweep + deferred + teardown_cap). Копия этой суммы в slotobs
// стала бы второй правдой и разъехалась бы при первой правке любого слагаемого —
// а история проекта показывает, что расходятся они молча (лог, врущий в 1.6 раза
// про бюджет, жил месяцами). Пакет slotobs НЕ зависит от client и зависеть не
// должен; направление зависимости сохранено.
//
// # Почему граница именно worstCaseTeardown, без коэффициентов
//
// Обоснование не «с запасом на глаз» (hard rule 8 такое запрещает), а физическое:
// worstCaseTeardown — это полное время жизни ХУДШЕЙ ячейки от готовности
// соединения до принудительного разрыва НАМИ. Слот старше этой величины означает,
// что не сработал наш собственный механизм ротации; на нашем пороге такой слот
// не мог быть срезан посредником, потому что мы обязаны были снять его раньше.
// Наблюдение описывает наш сбой, а не поведение цензора.
//
// Никакого множителя вида «1.5×» здесь нет намеренно: он был бы подгонкой. Запас
// в границу уже встроен её происхождением — граница ВКЛЮЧАЮЩАЯ (отбраковка при
// строгом `>`), потому что `slot_age` считается с ГОТОВНОСТИ соединения, а не с
// SYN (hard rule 12): TCP+TLS handshake, в поле 0.2–0.5 с, в величину не входит,
// поэтому реальная экспозиция на wire на столько же больше измеренной. Строгое
// «>=» срезало бы наблюдение, которое в бюджет как раз укладывается.
//
// teardownMs <= 0 отключает верхнюю границу — поведение до 2026-08-17. Отключение
// нужно явным: при выключенном gracefulDrain бюджет не описывает реальный путь
// ячейки, и граница из него была бы вымыслом.
func (r *Recorder) InferWithMinAgeAndTeardown(minAgeMs, teardownMs int64) Verdict {
	obs := r.Snapshot()

	clean, rej := filterNoise(obs, minAgeMs, teardownMs)
...
// Верхняя граница (teardownMs) добавлена 2026-08-17 по тем же соображениям, но
// с другой стороны распределения: наблюдение старше полного бюджета ротации
// говорит о НАШЕМ несработавшем механизме, а не о цензоре. Пять таких наблюдений
// (177–505 с, окно заморозки вывода) подняли CV возраста с 0.081 до 0.741 — то
// есть впятеро, и по этой величине выбирается ОСЬ.
func filterNoise(obs []Observation, minAgeMs, teardownMs int64) (clean []Observation, rej RejectionStats) {
	for _, o := range obs {
		switch {
		case o.AgeMs <= 0:
			// Смерть в момент подключения — сетевой отказ, не age-cut.
			rej.ZeroAge++
		case teardownMs > 0 && o.AgeMs > teardownMs:
			// Возраст старше полного бюджета жизни ячейки. Такой слот обязан был
			// быть снят НАМИ; на нашем пороге посредник не мог его срезать.
			// Наблюдение описывает сбой нашей ротации, а не поведение цензора.
			//
			// Стоит ВЫШЕ проверок по CloseKind намеренно: причина отбраковки
			// должна быть той, что информативнее для читателя лога. Возраст 293 с
			// с меткой close_other интересен именно возрастом — «локальное
			// закрытие» или «таймаут» на такой величине объясняли бы не то.
			rej.AboveTeardown++
