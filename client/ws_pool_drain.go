...
// claimFreeCellForHealing резервирует свободную ячейку для ВОСПОЛНЕНИЯ ёмкости —
// но только если ёмкости действительно не хватает. Возвращает -1, когда лечить
// нечего.
//
// # Зачем отдельно от claimFreeSlot
//
// claimFreeSlot безусловен: он берёт первую nil-ячейку, потому что вызывается из
// startDrain, где замена нужна по определению. Здесь условие обязательно. Утечка
// (замер 2026-08-14, D1) теряла ячейки по одной, и лечение должно ровно закрывать
// недостачу: подняться «на всякий случай» значило бы добавить лишнее TLS-соединение
// к origin, а темп новых соединений — это P0-угроза (policing по числу TLS).
// Замер того же дня: 388 conn/ч против 305 в базе, причём на живую ячейку темп не
// изменился (+1.6%) — весь рост от того, что пул держал штатные 8 вместо 6. То есть
// цена ёмкости реальна и платить её сверх нужного нельзя.
//
// # Что считается недостачей
//
// Цель — poolSize живых или поднимающихся слотов (slotReady + slotConnecting).
// Дренирующиеся НЕ считаются: у них уже есть replacement, и учёт их как живых
// занизил бы недостачу ровно на число параллельных дренажей — то есть лечение
// молчало бы именно в тот момент, когда дренаж и забрал ячейку.
//
// Мёртвые тоже не считаются: за ними стоит своя цепочка reconnectLoop, и её
// прерывание — как раз тот случай, ради которого эта функция существует. Двойного
// лечения не будет: конкурентный вызов не найдёт недостачи, так как placeholder
// уже установлен под reserveMu.
//
// # Почему ячейка НЕ захватывается placeholder'ом
//
// Соблазн поставить slotConnecting сразу, как делает claimFreeSlot, разобран и
// отвергнут: вызывающий (reconnectLoopInner) после этого идёт на новый круг
// цикла, где его ждут meltdown-пауза и backoff-лестница — секунды и десятки
// секунд. Placeholder всё это время считался бы «здоровым» в подсчёте выше и
// глушил бы повторное лечение, а при ctx.Done() посреди паузы — утёк бы как
// вечный slotConnecting, то есть правка против утечки завела бы утечку своего
...
func accrueAgeExposure(slot *poolSlot) time.Duration {
	...
	// Один учёт на слот, кем бы он ни был снят (ревью 2026-08-17). CAS стоит
	// ПОСЛЕ вычисления возраста и ДО накопления, поэтому возраст возвращается
	// для лога всегда — его печатают все ветки teardown, — а в счётчик перебег
	// попадает ровно один раз. Раньше от двойного счёта защищал фильтр по
	// slotDeathCause в вызывающем, и он терял два пути эвикции: см.
	// accrueAgeExposureOnDeath.
	if !slot.ageExposureAccrued.CompareAndSwap(false, true) {
		return age
	}
...
//
// ⚠ Развязка с двойным счётом идёт через флаг `slot.ageExposureAccrued`, а НЕ
// через фильтр по slotDeathCause. Первая версия (2026-08-17) отбивала
// deathCauseDrainTeardown, считая эту причину синонимом «tearDown уже
// посчитал». Ревью показало, что синонимии нет: причина приходит в
// handleSlotDeath из ТРЁХ мест — tearDown (:1093), tryForceEvictIdleSlot (:432),
// tryEmergencyEvictMinStreamsSlot (:538), — а accrueAgeExposure стоит только в
// первом, так что оба эвикта теряли учёт. Причина отвечает на вопрос «кто снял
// слот»; нужен ответ на «посчитано ли уже», и его знает только сам слот.
//
// Цена той ошибки была ровно в горячей части выборки: эвикты зовут при нехватке
// свободных ячеек, то есть по старым слотам под давлением, где перебег за
// порогом максимален.
//
// Двойной счёт по-прежнему исключён, и теперь структурно, а не по перечислению
// причин: CAS(false→true) пропускает ровно один вызов на слот, кто бы ни пришёл
// первым. Это важнее симметрии — недосчёт занижает риск, а двойной счёт создал бы
// ложный тренд роста, по которому стали бы двигать константы (hard rule 8).
func (p *WSPoolTransport) accrueAgeExposureOnDeath(slot *poolSlot) {
	if slot == nil {
		return
	}
