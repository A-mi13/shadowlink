...
//     width ~1/step.
//
// ⚠ ПОСЛЕДНЕЕ СВОЙСТВО ОПРОВЕРГНУТО ЗАМЕРОМ 2026-08-14. Здесь стояло
// продолжение: «A passive observer collecting ≥100 inter-arrival samples can no
// longer reject the "irregular" null hypothesis». Замер по логу
// nixavpn-DEBUG-20260813-153638 (2.04ч, 588 age-ротаций):
//
//	все 588 ротаций — в ОДНОЙ фазе сетки 5.000s (rotationWatchdogTick)
...
// предельно узким. Причина: этот джиттер прибавляется к ПОРОГУ, а порог
// сравнивается с возрастом только на тике watchdog'а. Поэтому джиттер решает,
// ЧЕРЕЗ КАКОЙ ТИК слот уйдёт, и не влияет на подтиковую фазу — а наблюдателю на
// краю ISP видна именно фаза.
//
// Побочное следствие того же квантования: при staggerStep=1s соседние offsets
// неразличимы. В поле видны ровно ДВА режима возраста ротации с разницей 5.0s
// (cell 0-4 → p50 73.8-74.7s, cell 5-8 → 79.6-79.7s), а не лестница из 16
// ступеней. Модель подтверждена на втором прогоне (083912, base 75s):
...
// teardowns.
//
// ⚠ Оба абзаца выше описывают КОНФИГУРАЦИЮ step=6s / cap=45s и к текущему
// проду (step=1s, cap=15s) неприменимы. Разбор 2026-08-14: при step=1s и 16
// ячейках максимум idx=15 даёт base=15s, а кламп требует `base > cap`, то есть
// при cap=15s он НЕ СРАБАТЫВАЕТ НИ НА ОДНОЙ ячейке. Склейки высоких idx в одно
// значение в проде нет, и cap не является причиной заходов в опасную полосу
// возраста — причина в offsets 5…9s у ячеек 5-9 плюс квантование тиком.
//
// Практическое следствие, чтобы вопрос не поднимался снова: правка cap
// 15s → 10s связала бы только idx 11…15, а их занятость в поле — 0.16%
// (по ячейкам: 0:87 1:84 2:83 3:84 4:82 5:80 6:75 7:31 8:16 9:2 10:3 11:1,
// 12-15 пусты). После квантования тиком корзину меняет только idx=15 → ноль
// соединений за прогон. То есть правка — no-op, а по hard rule 8 каждая
// тайминговая правка оплачивается замером.
//
...
//	   дрейфа за 2 часа нет: mean(первые 100) == mean(последние 100) == 0.000
//	92.2% TCP-connect'ов — в одном односекундном окне этой фазы
...
//
// ⚠ ОБОСНОВАНИЕ ПЕРЕПИСАНО 2026-08-12: прежние числа не воспроизводятся.
//
// Здесь стояло: «n=766, p90=118.0s, max=122.0s; сумма даёт 141.5s и границу
// держит; max() дала бы 111.5s, её пробивает уже p90 — ~10% выборки». Довод
// через p90 на новых данных НЕВЕРЕН. Три прогона восстановленных времён жизни:
//
//	20260811-145918  n=2219  p50=60.2  p90=85.2  p99=109.7  max=127.6
//	20260812-110200  n=744   p50=81.2  p90=86.2  p99=92.8   max=104.5
//	20260812-140930  n=607   p50=81.2  p90=86.2  p99=103.4  max=113.6
//
// p90=118.0s не воспроизвёлся НИ РАЗУ (везде 85–86s), то есть max()=111.5s
// никаким p90 не пробивается. Числа 118/122 остались от прогона с другой
// конфигурацией stagger и в текущей не имеют силы.
//
// Вывод «сумма, а не max()» тем не менее ОСТАЁТСЯ ВЕРНЫМ, но опирается на
// другое: слагаемые расходуются ПОСЛЕДОВАТЕЛЬНО (три шага выше), и это свойство
// кода, а не выборки. Замером его не опровергнуть — недостижение суммы означает
...
//
// Частая ветка — storm-brake (drainStormBrakeBackoff, 5s); редкая —
// no-free-cell (30s). В бюджет идёт РЕДКАЯ, потому что она хуже: слот уже
// перешагнул порог и признан подлежащим замене, но возвращён в slotReady — то
// есть продолжает принимать новые стримы (фильтр `getState() != slotReady`) и
// при этом запрещён к ротации 30 полных секунд. Именно такая ячейка стареет
// вглубь окна реза. «Учтено в счётчике no-free-cell» заменой не является:
...
// полу, а не выше него.
//
// ⚠ ВЫВОД ВЫШЕ ПЕРЕСМОТРЕН 2026-08-14. Здесь стояло: «Это не деградация: 25
// отложек по 5s за 2ч, потерь нет (балансы socks_connects=uplink=downlink=694,
// decrypt_fails=0). Гейт делает ровно то, для чего написан». Первая половина
// верна — потерь по балансам действительно нет. Вторая неверна.
//
// Разбор всех четырёх резов того же прогона по контексту (окно `reader started`
// → `reader error` для той же ячейки):
//
//	slot 8 · жизнь 86.6s · +79.6s `drain deferred (capacity floor)` → рез +7.0s
//	slot 6 · жизнь 84.4s · +79.8s то же                            → рез +4.6s
//	slot 5 · жизнь 82.7s · +79.7s то же                            → рез +3.0s
//	slot 10 · жизнь 83.7s · отложек нет, ячейка idx=10 доросла до порога
//
// ТРИ РЕЗА ИЗ ЧЕТЫРЁХ — на конце цепочки отложки. Ротация срабатывала вовремя,
// на своём тике 79.7s; её отказывал этот гейт с backoff 5s, и слот доживал до
// полосы, где hazard впервые перестаёт быть нулём. Резов среди отложенных
// 3/22 = 13.6% против базового 4/622 = 0.64%, относительный риск ×21
...
//
// ⚠ ПРИЧИНА НАЗВАНА НЕВЕРНО, исправлено 2026-08-14 по хронологии того же лога.
//
// Здесь стояло: «floor = floor(8 × 0.75) = 6, а сокращение порога 75s → 70s
// сдвинуло рабочую точку пула РОВНО НА этот пол». Распределения alive приводились
// верно (порог 70s: alive=5:15 alive=6:146 alive=7:75 alive=8:8 против порога 75s:
// alive=6:26 alive=7:203 alive=8:95), но вывод о причинности из них не следует.
//
// Хронология решает вопрос:
//
//	15:42:12 первая ПОТЕРЯ ячейки (empty 8 → 9)
//	15:44:32 первая отложка гейта, и все 22 — при ready=5 floor=6
//	до 15:42:12 пул шёл на 8 живых, гейт не срабатывал НИ РАЗУ
//
// ready=5 при poolSize=8 достижимо только после потери двух ячеек. Порог 70s к
// этому отношения не имеет — пул посадила УТЕЧКА ЯЧЕЕК в reconnect/recycle
// (см. reconnectLoopInner, ветка recycle guard): empty 8 → 9 → 10 → 11,
// mean alive 7.21 → 5.80 за два часа.
//
// Настоящая цепочка: утечка → alive падает до пола → гейт отклоняет плановую
// ротацию → слот доживает до полосы ненулевого hazard → рез посредника. Три реза
// из четырёх в том прогоне сидят на её конце. Гейт — предпоследнее звено, а не
// первое, и «делает ровно то, для чего написан» скрывало не порог, а утечку.
//
// Практические следствия:
//   - утечка исправлена (лечение ёмкости в reconnectLoopInner), и это снимает
//     ПЕРВОПРИЧИНУ. В прогоне 2026-08-14 отложек уже 0 при неизменной доле 0.75;
//   - A/B по доле в прежнем виде ОБЕСЦЕНЕН: критерий
//     «capacity_floor_deferred_total → 0» выполняется без всякой правки доли, как
//     только пул перестаёт терять ячейки. Замер зачёл бы эффект не той правке;
//   - понижать долю (0.75 → 0.5) сейчас нельзя: это ЗАМАСКИРОВАЛО бы остаточную
//     утечку, если она где-то ещё есть, и приблизило бы clinch, ради которого
//     гейт и заводился.
//
// Значение остаётся 0.75. Ручка (SHADOWLINK_READY_CAPACITY_FLOOR_FRACTION) —
// только для замера, и осмысленный критерий теперь другой: число отложек при
// ready < floor НА ПРОГОНАХ, где empty > 8, то есть где ёмкость всё-таки просела.
//
// Spec 2026-05-24 (concurrency-lift-and-backoff).
...
// клампятся: доля 1.0 означала бы «все слоты обязаны быть ready», то есть при
// любой ротации (слот выходит из slotReady) гейт запрещал бы следующую — пул
...
type WSPoolTransport struct {
	...
	// что при пороге 70s пул штатно балансирует НА полу, и отложки гейта
	// удлиняют жизнь слота в полосу ненулевого hazard (см. константу).
	readyCapacityFloorFraction float64
	...
	// lastHazardLog — то же для hazard-кривой (хранит hazardLogKey). Кривая
	// меняется только при новых наблюдениях, а stats-логгер зовёт нас каждые 5s.
	lastHazardLog atomic.Value
	...
	//
	// ⚠ 2026-08-13: агрегат СОХРАНЁН, но в health-строку больше не печатается
	// под этим именем. Он складывал наши плановые ротации с резами посредника,
	// а имя `slot_deaths_1m` рядом с `rotations_1m` читалось как «пул умирает».
	// Дважды подряд дало НЕВЕРНЫЙ диагноз (аудит 2026-08-13 и разбор инженера
	// в тот же день: значение 18 прочли как «перегрузка/meltdown», хотя это был
	// штатный темп ротации). Разбивка — в slotTeardowns1m / slotCuts1m ниже;
	// агрегат остаётся как их сумма для внутренних потребителей.
	slotDeaths1m atomic.Int32

	// slotTeardowns1m — смерти по НАШЕМУ решению за ~60s: плановая ротация и
	// graceful-drain teardown. Это успех политики ротации, а не отказ.
	//
	// slotCuts1m — смерти, которые решили ЗА НАС: natural (сеть/origin) и
	// age_cut (рез посредника). Только это поле означает «что-то не так».
	//
	// Почему два поля, а не разбивка по всем четырём причинам: вопрос
	// оператора бинарный — «это мы или нас?». Полная разбивка добавила бы
	// состояние без операционной ценности, а причина каждой смерти и так
	// печатается в строке самой смерти.
	//
	// ⚠ Граница проведена по ИСТОЧНИКУ РЕШЕНИЯ, а не по «плохо/хорошо»:
	// deathCauseAgeCut в диспетчере считается штатным (fast reconnect, не
	// кормит meltdown), но решение принял посредник — значит это рез.
	// Именно эта величина отвечает на вопрос «где окно реза», ради которого
	// делался замер 2026-08-13.
	slotTeardowns1m atomic.Int32
	slotCuts1m      atomic.Int32
...
type WSPoolConfig struct {
	...
	//
	// Выведена в конфиг 2026-08-14 ради A/B, а не для повседневного тюнинга:
	// разбор показал, что при MaxSlotAge=70s пул штатно балансирует НА полу и
	// отложки гейта удлиняют жизнь слота в полосу ненулевого hazard (3 реза из 4
	// в прогоне — на конце такой цепочки). Занижение пола — это риск clinch'а,
	// ради которого гейт и заводился, поэтому дефолт НЕ изменён. Обоснование и
	// критерий успеха замера — у константы readyCapacityFloorFraction.
	ReadyCapacityFloorFraction float64
...
func (p *WSPoolTransport) rotationWatchdogLoop() {
	// Timer с перевзводом, а НЕ Ticker (2026-08-14, замер фазы).
	//
	// Ticker просыпается на жёсткой решётке k*period с фазой, постоянной на весь
	// сеанс, — и эта фаза уходила на wire. Замер прогона 08-13: все 588
	// age-ротаций в одной фазе 5-секундной сетки, σ 0.0001s, 92.2% TCP-connect'ов
	// в одном 1-секундном окне.
	//
	// Срезка периода 5s → 500ms убила низкочастотную линию (vector strength на
	// периоде 5s: 1.0000 → 0.0096), но решётку как таковую не убрала: R на периоде
	// 500ms остался 0.8657 по моменту дренажа. Пик не размазался, он ПЕРЕЕХАЛ с
	// 0.2 Гц на 2 Гц. Любое дальнейшее уменьшение периода воспроизвело бы тот же
	...
	//
	// ⚠ ЭТОТ ДЖИТТЕР — ЯКОРЬ ФАЗЫ ДЛЯ ВСЕГО ПУЛА, а не только для свипа.
	//
	// Ревью предсказывало, что момент ЗАКРЫТИЯ останется на решётке: фактический
	// teardown делает drainWatchdog по своему time.NewTicker(drainPollInterval =
	// 500ms), и тот жёсткий. Замер прогона 161915 предсказание опроверг и показал
	// механизм:
	//
	...
	//
	// Ticker дренажа создаётся ВНУТРИ каждого drainWatchdog, поэтому его фаза
	// наследуется от момента startDrain — а startDrain зовётся отсюда. Якорь
	// джиттерован → абсолютная фаза на wire размазана, хотя относительная сетка
	// сохранена идеально.
	//
	// Следствие — зависимость, а не свобода: возврат к time.NewTicker ЗДЕСЬ
	// немедленно проявит решётку 500 мс в моментах закрытия, хотя код дренажа не
	// изменится. Ровно это наблюдалось в прогоне 115514: R=0.8933 по close при
	// жёстком свипе и том же drainPollInterval.
	//
	// Остаточный признак, замером НЕ снятый: медиана (close − start) = 0.500s
	// ровно, то есть пара «новое TLS → close через полсекунды» жёстко
	// коррелирована по ИНТЕРВАЛУ. Это шаблон, а не периодичность; отдельная работа.
	for {
...
func (p *WSPoolTransport) emitHealthSummary() {
	...
	p.log.Info("WS pool health",
		...
		// Смерти слотов за ~60s, РАЗДЕЛЬНО по источнику решения. Раньше здесь
		// стояло одно поле `slot_deaths_1m` со всеми причинами сразу — оно
		// складывало наши плановые ротации с резами посредника и рядом с
		// `rotations_1m` читалось как «пул умирает». Замер 2026-08-13 показал
		// пропорцию: 1326 плановых против 125 резов, то есть агрегат на
		// 91% состоял из здоровой работы. Дважды подряд дал неверный диагноз
		// («перегрузка/meltdown» на значении 18, которое было темпом ротации).
		//
		// slot_cuts_1m — единственное из двух, что означает «что-то не так»:
		// natural (сеть/origin лёг) + age_cut (рез посредника). Полный отказ
		// пула 2026-08-11, ради видимости которого счётчик и заводился,
		// целиком попадает СЮДА и не разбавлен ротациями.
		"slot_cuts_1m", p.slotCuts1m.Load(),
		// slot_teardowns_1m — наши собственные снятия слота (плановая ротация
		// + graceful-drain teardown). Ожидаемо НЕнулевое на здоровом пуле.
		"slot_teardowns_1m", p.slotTeardowns1m.Load(),
		...
		// Лечение ёмкости (D1, 2026-08-14). Ненулевой retarget = утечка ячейки
		// произошла бы, но вылечена; gave_up = пул идёт на ячейку меньше.
		// Читать вместе с empty/alive выше: раньше утечка проявлялась как
...
// sweepWorstCase — вклад watchdog-свипа в worst-case время жизни соединения.
//
// ЕДИНСТВЕННЫЙ источник этой величины: и бюджет, и лог, и тесты обязаны спрашивать
// здесь. Складывать слагаемые по месту нельзя — так родился H-15 (см.
// worstCaseTeardown).
//
// # Формула: 2 × (tick + jitter_max) = 4 × rotationWatchdogTick
//
// Свип просыпается через `tick + uniform[0, tick)`, то есть максимум 2×tick на
// одно пробуждение. Пробуждений в худшем пути ДВА:
//
//  1. обнаружение — слот перешагнул порог сразу ПОСЛЕ пробуждения, поэтому до
//     ближайшей проверки ждёт полный интервал;
//  2. действие — если на этой проверке startDrain отложил ротацию (storm-brake
//     ставит nextDrainAttemptNs), следующая попытка возможна только на очередном
//     пробуждении: свип — единственный, кто читает эту отсрочку.
//
// Итого 2 × 2 × tick = 4 × tick = 2s при tick=500ms.
//
// # История двух ошибок в этом слагаемом
//
// До 2026-08-14 считался ОДИН тик: занижение ровно на период, потому что второе
// пробуждение (после отсрочки гейта) не учитывалось вовсе. Слагаемое `deferred` в
// бюджете было, а тик, через который отсрочка физически рассасывается, — нет.
//
// Затем джиттер на пробуждении сделал прежнюю формулу `2 × tick` заниженной уже
// по другой причине: интервал перестал быть постоянным. Берём верхнюю границу
// (2×tick на пробуждение), а не среднюю (1.5×tick): бюджет обязан быть верхней
// границей, занижение здесь по памяти проекта хуже завышения.
...
// переприцелиться на другую ячейку, когда её собственную забрал дренаж.
//
// Зачем предел: без него цепочка в теории гоняется за ячейками бесконечно — на
// каждом круге дренаж успевает занять очередную свободную, лечение находит
// следующую. Горутина живёт вечно, и это была бы утечка своего вида на месте
// исправленной.
//
// 3 — с большим запасом против наблюдаемого темпа: в полевом логе 7 отказов
// connect (то есть 7 поводов войти в эту ветку) против ~590 дренажей за 2ч, и
// повторный обрыв ОДНОЙ цепочки не встретился ни разу. Исчерпание предела значит
// «пул идёт на одну ячейку меньше до следующей естественной смерти слота» —
// деградация, но не потеря: у той смерти будет своя цепочка reconnectLoop.
// Событие логируется WARN'ом и считается в HealingRetargetGaveUpTotal, потому что
// молчаливое исчерпание невозможно было бы отличить от исходной утечки.
const maxHealingRetargets = 3
...
func (p *WSPoolTransport) reconnectLoopInner(idx int, fastFirstAttempt bool) {
	...
	// retargets — сколько раз эта цепочка переприцеливалась на другую ячейку,
	// потому что её собственную забрал дренаж. См. maxHealingRetargets и разбор
	// утечки в ветке recycle guard ниже.
	retargets := 0
	for attempt := 0; ; attempt++ {
		...
		p.reserveMu.Unlock()
		if recycled {
			// ⚠ ОТКАЗ ОТ ЭТОЙ ячейки — не то же, что отказ от ВОССТАНОВЛЕНИЯ
			// ёмкости (утечка найдена замером 2026-08-14, D1).
			//
			// До правки здесь стоял голый return. Он верен в своей части: наш
			// reconnect устарел, ячейку уже занял дренаж, писать в неё нельзя.
			// Но никто другой ёмкость не восполнял: периодического healer'а в
			// пуле НЕТ (единственный вызов reconnectLoop на старте — ws_pool.go
			// ~2386, это начальный connect, а не лечение). Каждая цепочка
			// reconnectLoop одноразова, и обрыв здесь терял ячейку НАВСЕГДА.
			//
			// Полевой след (лог nixavpn-DEBUG-20260813-153638):
			//
			//	15:41:14 WARN reserve slot connect failed — placeholder freed slot=0
			//	15:41:14 reconnecting slot 0 backoff=7.6s
			//	15:41:17 drain slot 1 → replacement_slot=0   ← дренаж забрал ячейку
			//	15:41:18 reconnect short-circuited            ← reconnect отменён
			//	→ empty 8 → 9 → 10 → 11, mean alive 7.21 → 5.80 за 2 часа
			//
			// Цена не в самой ёмкости, а в том, что стоит НА КОНЦЕ цепочки:
			// alive падает до пола storm-brake (ready=5 при floor=6), гейт
			// начинает отклонять плановые ротации, слот доживает до полосы
			// ненулевого hazard и его режет посредник. Разбор того же лога: до
			// первой потери ячейки (15:42:12) гейт не срабатывал НИ РАЗУ, первая
			// отложка — 15:44:32, и все 22 при ready=5. То есть «порог 70s
			// посадил пул на пол» (прежний диагноз) неверно: пул посадила утечка.
			//
			// Поэтому: от ячейки отказываемся, от ёмкости — нет. Ищем ЛЮБУЮ
			// свободную и поднимаем на ней замену.
			// Ограничение числа переприцеливаний — защита от того, что цепочка
			// будет вечно гоняться за ячейками в горячем цикле дренажей. Без
			// него правка против утечки могла бы дать живую горутину, которая
			// никогда не завершается: каждый круг дренаж успевает забрать
			// очередную свободную ячейку, лечение находит следующую, и так далее.
			// 3 попытки — с запасом против наблюдаемого темпа (7 отказов connect
			// за 2ч против ~590 дренажей); дальше отдаём ёмкость следующей
			// естественной смерти слота, у которой будет своя цепочка.
			if retargets >= maxHealingRetargets {
				p.log.Warn("WS pool reconnect abandoned — cell kept being recycled",
...
// в диспетчере рядом с ветвлением по cause и не разъезжался с ним.
//
// ⚠ 2026-08-13: причина ТЕПЕРЬ ИСПОЛЬЗУЕТСЯ. До этой правки параметр был
// `_ slotDeathCause` — счётчик оставался агрегатным, и это дважды подряд
// дало неверный диагноз: значение читали как «пул умирает», хотя 95-96%
// его составляли наши же плановые ротации (замер 2026-08-13: 1326 плановых
// против 125 резов). Агрегат сохранён для совместимости, но наблюдаемость
// теперь идёт через разбивку — см. докстринги полей.
//
// Классификация по ИСТОЧНИКУ РЕШЕНИЯ:
//
//	наши    — deathCausePreemptiveRotation, deathCauseDrainTeardown
//	не наши — deathCauseNatural, deathCauseAgeCut
//
// default попадает в резы намеренно: неизвестная причина не должна тихо
// уезжать в «это мы, всё нормально». Новая причина, добавленная без правки
// этого switch, будет шуметь — и это лучше, чем молчать.
//
...
func (p *WSPoolTransport) handleStreamClose(cl *Client, streamID uint16) {
	// Декремент счётчика слота ПЕРЕД удалением из карты — причина фантомного
	// счётчика, найдена 2026-08-12.
	//
	// AssignStream делает streams.Add(1) + streamMap.Store. Освобождение шло
	// двумя путями, а декремент был только в одном: ReleaseStream его делал,
	// handleStreamClose — нет. При этом второй путь не редкость, это штатное
	// завершение стрима по FlagStreamClose от сервера (:3719, смерть origin TCP)
	// плюс ветка неразбираемого migration-кадра (:4139).
	//
	// Полевая цена: counter_streams>0 при map_streams=0 в 28.1% дренажей прогона
	// 20260812-110200 и 34.4% прогона 20260812-140930 (z=2.48 — доля РОСЛА).
