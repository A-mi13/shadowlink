...
// (engine/engine.go:216), поэтому безусловная печать дала бы 720 строк в час
...
func TestSlotDeathGate_ZeroSamplesSpeaks(t *testing.T) {
	...
	// выборки, которой здесь нет. Нулём он читался бы как «дефицит 142.5s»
	// (бюджет прода после джиттера свипа; до него было 141.5s).
...
func TestSlotDeathGate_ReportsPlannedDenominator(t *testing.T) {
	...
	// Две доли раздельно: по пожизненным счётчикам и по содержимому рингов. Под
	// одним именем `cut_share` их путали (ревью 2026-08-12).
	//
	// ⚠ Ожидания обновлены 2026-08-14: ёмкости рингов разведены
	// (PlannedCapacity=4096 против DefaultCapacity=512), поэтому 740 плановых
	// больше НЕ обрезаются и обе доли совпадают — 8/748. Раньше ring-доля давала
	// 0.0154 против lifetime 0.0107, и это расхождение было не свойством метрики,
	// а дефектом ёмкости: в hazard то же самое обрезание давало завышение ×2.06.
	//
	// Смысл теста сохранён: обе величины печатаются порознь и обязаны сходиться,
	// пока ринг не переполнен. Расхождение теперь означает переполнение, а не
	// норму, — и именно в этот момент читать ring-долю нельзя.
	for _, want := range []string{
		"planned_rotations=740",
		"cut_share_lifetime=0.0107",
		"cut_share_ring=0.0107",
		"cut_share_ring_denom=748",
	} {
		if !strings.Contains(out, want) {
			t.Errorf("в строке нехватки данных нет %q:\n%s", want, out)
		}
