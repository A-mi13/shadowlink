...
**Обновлено 2026-08-31 (v0.3.0).** Статус подтверждён независимо: вне тестов не
вызывается ни `Rekey`, ни `RekeyNeeded`; серверный `handleControl` — заглушка над
keepalive, несмотря на комментарий «Control chunks handle rekeying». Дефер
остаётся, но два обстоятельства изменились:

1. **Ложное обещание снято из докстрингов.** До правки `Rekey`/`RekeyNeeded`
   документировали политику «раз в час либо при `sendSeq` → 2^32», которой никто
   не исполняет — читающий исходник получал неверное представление о гарантиях.
   Теперь там прямо сказано, что механизм не подключён, и назван этот блокер.
2. **Сторож `TestRekey_NotWiredInProduction`** (`core/rekey_status_test.go`)
   краснеет в ОБЕ стороны: при появлении прод-вызова (с указанием, что сначала
   надо закрыть C13) и при удалении предупреждения из докстринга. То есть C13
   больше не может быть обойдён молча.

Уточнение к «future hazard»: forward secrecy де-факто есть — её даёт ротация
слотов, где каждый слот делает свой handshake. Чего ротация НЕ покрывает — гейт
переполнения `sendSeq` (он per-session), и `core/chunk.go` опирается на
предположение «rekey at ~2^32 well before wrap», которое живой код не
обеспечивает. Недостижимость переполнения держится на тайминговых константах, а
не на механизме.
