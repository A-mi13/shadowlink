...
### H-2. ALPS-расхождение cold-path (utls) vs hot-path (bogdanfinn) для chrome133
**[ПРОВЕРЕНО чтением].** `client/connmanager.go:206` содержит только
`WithForceHttp1()`; **`WithDisableHttp3()` отсутствует**.

| Поверхность | cold utls 1.8.3 | hot bogdanfinn 1.14.0 |
|---|---|---|
| ALPN | `http/1.1` | `http/1.1` ✅ |
| **ALPS** | `["h2"]` | **`["h3","h2"]`** ❌ |
| **Порядок extensions** | шафлится per-dial | фиксирован ❌ |

chrome120 и chrome131 согласованы — дефект специфичен для chrome133, который
несёт **60%** веса пула (`skins/browser/profile.go:64`). ALPS и порядок extensions —
прямые входы JA4. Реальный Chrome никогда не объявляет ALPS `h3` при ALPN
`http/1.1`, и Chrome 110+ шафлит extensions на каждом соединении.

Почему lockstep не поймал: `validateProfile` сверяет **числа мажоров** (оба
профиля честно рапортуют 133), а не **байты на проводе**. Это ровно тот класс
расхождения, который `fingerprint.go:46-51` объявляет главным риском.

Фикс: одна строка `WithDisableHttp3()` + байтовый тест ALPS/ALPN-парности на все
три профиля.
