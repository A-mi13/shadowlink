...
## Фазовый замок свипа, hazard по экспозиции, capacity-floor (2026-08-14)

Независимая проверка (Fable) разбора от 08-13 по первичным логам. Реконструкция
622 ротаций воспроизведена бин в бин, но исходный вопрос («срезать ли
`STAGGER_OFFSET_CAP` 15s → 10s») снят по другому основанию, а по пути найдены две
вещи крупнее него. Полный разбор — `docs/plans/2026-08-14-post-fable-fixes.md`.

### Status

- **`SHADOWLINK_SWEEP_PHASE_JITTER`** (новый, дефолт **4s**, `-1s` выключает) —
  подтиковое размазывание МОМЕНТА дренажа. Замер `nixavpn-DEBUG-20260813-153638`:
  все 588 age-ротаций в одной фазе 5s-сетки (σ 0.0001s, дрейфа за 2ч нет), 92.2%
  TCP-connect'ов в одном 1s-окне. Комментарий про размазанный FFT-пик описывал
  механизм, который не исполнялся: stagger правит ПОРОГ, а порог проверяется
  только на тике `rotationWatchdogTick`. Реализовано через существующий
  `nextDrainAttemptNs`, однократность — флаг `phaseJitterApplied` на `*poolSlot`.
- **`SHADOWLINK_READY_CAPACITY_FLOOR_FRACTION`** (новый, дефолт 0.75 —
  **не изменён**) — выведен ради A/B. 3 реза из 4 в прогоне следуют за отложкой
  ...
  сдвинул рабочую точку пула РОВНО на пол (`alive=6` при `floor=6`).
- **`HazardBand.RateByExposure()` + `ExposureMs` + `RingSaturated`** — прежний
  ...
  кромки: плановые проживают в полосе 0.2s, а не 2.5s), а переполнение ринга
  плановых давало завышение ×2.06 в PROBE. Обе формы печатаются рядом.
- **Лог:** `sweep_tick` → **`sweep_budget`** (величина = тик + джиттер; чистый
  тик и джиттер печатаются рядом порознь). Новые поля hazard-строки:
  `exposure_s`, `rate_exp`, `p_pass`, `rate_act`, `ring_saturated`. Новый
  счётчик `sweep_phase_jitter_applied_total` (+ Prometheus).
- **Прод-настройки `.bat` не менялись**: `MAX_SLOT_AGE=70s` как было.

### Rollback

1. Фазовый джиттер: `SHADOWLINK_SWEEP_PHASE_JITTER=-1s` — возвращает поведение
   до 08-14 РОВНО (отдельный выключатель, а не 0, потому что 0 занят дефолтом).
...
| `worstCaseTeardown()` прод | 145.5s | 149.5s (+4s джиттера, честно в бюджете) |
| hazard 80–85s | `rate_act` 3.37% | `rate_exp` 6.94% (0.0069 1/с) |
...
- `CLAUDE.md` правило 9 (уточнено: причина двух режимов — квантование тиком, а не
  «дремлющая лестница») и правило 10 (cap при `step=1s` — no-op)
