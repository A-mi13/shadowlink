...
- **Фаза развязана ЦЕЛИКОМ, включая момент close.** Прогноз «развязано наполовину,
  ...
  **Механизм — ЯКОРЬ**: сетка `drainPollInterval` жива на 100% относительно старта
  дренажа (R((close−drain_start) mod 500ms)=1.0000, 1525/1525 в одном 50ms-бине),
...
- **НОВОЕ: sticky — единственный оставшийся источник хвоста.** 77% экспозиции >82s
  (196.8 из 254s), 12 из 17 соединений >85s, max 102.6s. Все 50 teardown'ов —
  `age_backstop` и `drain_duration=25s` РОВНО, то есть каждый упёрся в
  stickyMaxDrainAge. Добавлены `slot_age` в sticky-лог (drain_duration в этой ветке
  константа и о риске молчит) и счётчик `sticky_exposure_over_82s_ms` (порог 82s:
  плановые массово идут через 80.2s и забили бы счётчик штатной работой).
  Константу НЕ трогали — вред не наступил (~1 ожидаемый рез за 4ч, наблюдалось 0).

### Rollback

1. `slot_age` / `sticky_exposure_over_82s_ms` — только наблюдаемость, поведение не
   меняют; удаление безопасно, но вернёт слепоту к единственному оставшемуся риску.
2. `stickyExposureThreshold` — порог НАБЛЮДЕНИЯ, менять можно без замера, но
   исторические значения счётчика станут несравнимыми.
...
| экспозиция >82s | 109.6 с/ч | 41.5 с/ч | 63.3 с/ч |
...
- `client/sticky_stream_test.go`: `TestStickyExposure_CountsOnlyOverThreshold`,
  `TestStickyExposureThreshold_AbovePlannedRotationBand`
