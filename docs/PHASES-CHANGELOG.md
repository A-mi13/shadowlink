...
- потолок задаёт **клиентское окно 1 МиБ** (`ws_pool.go`): 1 МиБ / 0.241 с =
  **34.8 Мбит/с**; наблюдаемые 23 — 66 % от него, остаток съедает возврат
  кредитов;
- одиночный TCP при 241 мс и обычном `tcp_rmem` дал бы ~199 Мбит/с — на порядок
  больше, то есть BDP не ограничитель;
- 8 × 23 ≈ 187 подтверждает: общего окна уровня сессии/слота нет.
...
| дефект возвращён | **0/4**, и счётчики впервые назвали ветку: `udp_drop_reasons="no_client_yet=4"` |

То есть подтверждены сразу три вещи: дефект реален, правка его снимает, и новая
наблюдаемость показывает причину — ровно то, чего не было, когда клиент видел 33 %.
