...
| `server/` | HTTP handler, tunnels, multiplexing, decoy, rate limiter, client auth |
...
Parser fuzz tests live in `skins/browser/request_fuzz_test.go`. CI nightly runs 1h budget per target:

```bash
go test -fuzz=FuzzParseDataPayload -fuzztime=1h ./skins/browser/
go test -fuzz=FuzzParseHandshakePayload -fuzztime=1h ./skins/browser/
go test -fuzz=FuzzBuildParseDataRoundTrip -fuzztime=1h ./skins/browser/
```

Required green before Phase 1 deploy (per Bearer→body-prefix migration spec §5.7).

## Race Detector

Concurrent server tests must pass under `-race` in CI:

```bash
go test -race -count=3 ./server/ ./client/ ./skins/...
```

Requires CGO + gcc. Windows dev hosts without gcc fall back to plain `go test` (counter-invariants still checked, data-race detection skipped). CI MUST run on Linux/macOS or a Windows host with gcc.
