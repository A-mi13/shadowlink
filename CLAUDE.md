...
9. **Возрасты смертей описывают НАШ порог, а не цензора** (аудит 2026-08-13,
   ...
   3. **`time.Timer` с перевзводом на `tick + uniform[0, tick)`** — так решётки не
      остаётся вовсе. Джиттер сэмплируется заново каждый взвод (один сэмпл на
      сеанс = решётка с другим шагом).
   ...
   наблюдаемость. Замером новая схема пока **не проверена**.
