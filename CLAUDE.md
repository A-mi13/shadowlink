...
8. Тайминговые константы — производные от TSPU age-окна ~130–190 с; не менять
   ...
   **объёму** (~16–20 КБ), а цифра 130–190 с похожа на перенос замера GFW.
   ⚠ И **глобальная константа неверна в принципе**: ACM IMC 2022 — ТСПУ
   неоднородна по регионам и операторам, поэтому «измерить и зашить» задачу не
   решает (P0 переформулирован 2026-07-31). Курс — адаптация per-AS на клиенте.
   Шаг 1 сделан: `client/slotobs` + метрики `shadowlink_slot_death_*` собирают
   распределение смертей слотов; ось реза определяется по `_cv` (у оси с жёстким
   порогом CV ниже). Наблюдение решений не принимает. План — `FIELD-CHECKS.md` §2.
