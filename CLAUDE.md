...
9. **Возрасты смертей описывают НАШ порог, а не цензора** (аудит 2026-08-13,
   ...
   ⚠ **Фаза свипа была приколочена**: все 588 age-ротаций прогона 153638 легли в
   одну фазу 5-секундной сетки (σ 0.0001 с, дрейфа за 2 ч нет), 92.2 %
   TCP-connect'ов — в одном односекундном окне. Комментарий про размазанный
   FFT-пик описывал механизм, который **не исполнялся**: stagger-джиттер
   прибавляется к порогу, а не к моменту действия. Лечится **только периодом
   тикера**: `rotationWatchdogTick` срезан 5 с → **500 мс** (2026-08-14), после
   чего амплитуда stagger-джиттера (±0.5 с при step=1s) впервые сравнима с
   квантом сетки. Попутно бюджет **удешевился** (слагаемое `sweep`: 2×5 с →
   2×0.5 с). ⚠ Отдельная ручка «джиттер на момент дренажа» была реализована и
   **откачена в тот же день**: отсрочка через `nextDrainAttemptNs` не работает,
   потому что единственный её читатель — сам свип, а он просыпается по тому же
   тикеру; отсрочка короче тика всегда переносит дренаж ровно на следующий тик,
   в ту же фазу. Не воскрешать без замера.
   ⚠ Остаточная решётка 500 мс **не проверена замером** — критерий в
   `docs/plans/2026-08-14-post-fable-fixes.md`.
   ⚠ `worstCaseTeardown()` в проде = **141.5 с** (70 + 15.5 + 2×0.5 + 30 + 25).
   Было 145.5 с при тике 5 с; **не 142.5** — та величина относилась к PROBE после
   адаптации базы до 67 с. Слагаемое `sweep` считает тик **дважды**: обнаружение
   перезревания и повторная попытка после отсрочки гейта (её читает только свип).
   Прежняя формула с одним тиком была занижена ровно на период — H-15 с другой
   стороны. В накладных `stagger` лишь 21 %, а `deferred 30 + tear 25` — 73 %;
   `drainRevertBackoff` в прод-прогоне не наблюдался ни разу. Сжимать бюджет
   через stagger/cap — неверная точка приложения.
