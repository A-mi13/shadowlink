...
9. **Возрасты смертей описывают НАШ порог, а не цензора** (аудит 2026-08-13,
   ...
   ⚠ Ladder stagger фактически дремлет: `slotStaggerOffset` берёт idx ячейки на
   момент connect, живых ячеек ~8 из 16 → реальные offsets ~4.7 и ~10.2 с, а не
   лестница 0…45 с. Поэтому «8 ячеек из 16 с порогом 114 с» (п. 8) — свойство
   прошлой конфигурации, в прогоне 2026-08-13 **не воспроизводится**:
   max порог 85.2 с. И `worstCaseTeardown()`=183 с завышен в 1.75× против
   наблюдаемого max 104.7 с — проверка «бюджет против p10 смертей» будет ругаться
   всегда, т.е. повторит H-15 с другой стороны.
   → Настоящее окно **не измерено**. План замера: `docs/plans/2026-08-13-age-wall-probe.md`.
