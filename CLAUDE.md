...
9. **Возрасты смертей описывают НАШ порог, а не цензора** (аудит 2026-08-13,
   ...
   3. **`time.Timer` с перевзводом на `tick + uniform[0, tick)`**
      ...
      шагом).
   ⚠ **Правка развязала момент НОВОГО СОЕДИНЕНИЯ, но не момент ЗАКРЫТИЯ**
   (ревью 2026-08-14). `startDrain` зовётся из свипа и сразу спавнит
   `connectReserveSlot`, поэтому `reader started` едет на джиттерованном
   расписании. А фактический teardown делает `drainWatchdog` по **своему**
   `time.NewTicker(drainPollInterval = 500 мс)` (`ws_pool_drain.go:805`), и он
   остался жёстким — на wire живёт вторая, независимая сетка 2 Гц по моментам
   close. Замер `R = 0.8657` брался «по моменту дренажа», то есть как раз по ней,
   и одной этой правкой упасть **не обязан**. Формулировка «решётки не остаётся
   вовсе» была сильнее данных.
   → Критерий фазы — **vector strength `R < 0.2` на периоде тика по
   `reader started`**, а НЕ «доля в модальном окне сетки 5 с»: та падает
   автоматически при любом тике меньше секунды, то есть измеряет нашу ручку, а не
   наблюдаемость. Замером новая схема пока **не проверена**, и для close нужен
   **отдельный** замер (и отдельная правка `drainPollInterval`).
